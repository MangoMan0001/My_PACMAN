...
3. [⛏追加要件](#3-追加要件)
	...
	7. [よかったこと・次に気を付けること](#3-7-よかったこと次に気を付けること)
4. [🌈リソース](#4-リソース)
	1. [参考URL](#4-1-参考url)
...
タイムライン・リスク分析・チーム分担・受入テスト等詳細なドキュメントは[`docs/`](docs/project_management_JA.md)(プロジェクト管理ディレクトリ)にまとめます。

### 3-7. よかったこと、次に気を付けること

#### ayhirose　

**よかったこと**
- GITが怖くなくなった。とてもありがたい。イシューとプルリクエスト、マージを経験してチーム課題の進め方の解像度がかなり上がった。  
- モデルの設計後に大幅な修正がなかった。引数の変更はいくつかあったもののオブジェクトを継承するOOP基本設計にブレが生じなかった。  

**次に気を付けること**  
- コミットを分ける習慣をつけること。レビューがしやすくなるので心がけたい。  
- 同一クラス内での分担作業を簡易的に行える方法を模索したい。  

#### nsato

**よかったこと**
- Git、GitHubのisuue、Pull Request、Project、Wiki等を初めてちゃんと使用しながらチームでの作業ができ、とても勉強になった。 個人での作業にも取り入れていきたい。  
- 初めにチームメンバーがクラス設計を綿密に行ってくれたおかげで、機能単位での作業分担がしやすくとても進めやすかった。 感謝です。  
- 上記の2点のおかげでプロジェクト全体としてストレスなく取り組めた。  
- 最終的な成果物としてゲームを作成できたこと、個人ではなくチームで取り組めたこと自体がとてもいい経験になった。  
- 自分とはアプローチの仕方を知ることができ参考になった。特にpydanticなど。

**次に気を付けること**
- チームについて  
    - メンバーに甘え頼りすぎ、タスクの比重が偏ってしまってないか心配だった。チーム内での実装スケジュールとは別に個人での実装目標日程なども立てて取り組み、自分に対するプロジェクトマネジメントを行うべきだった。次回に活かしたい。  
- 設計について  
    - 設計次第で例えばタスクの量が分かってスケジュールを組みやすくなる、どこから手を付ければいいかわかりやすい、コードやディレクトリ構成が汚れない、チームなら分担がしやすいなど、プロジェクトが進めやすくなることが分かったので意識的に学びたい。  
- プロジェクトについて  
    - Pacmanを実装するにあたって、仕様、機能をどの程度まで原作に準拠するか、必要最低限の実装にするか、隠しコマンドや画面の描画を凝るかなど凝れる部分が多くその実装も時間があればしたい。ただ今回は課題の一環として完成物を作成することが重要だと思うのでその点は区切りができてよかったと思う。  
- Git、GitHub系列の便利機能について  
    - issue、プルリクエスト、Project、Wiki等使いこなしたい。Worktreeというものがあるらしくそれも使いたい。  
- レビューについて  
    - レビューの仕方、フローを整えたい。AIを使用したレビューのテンプレートを作成する。  
- コーディングスキル以外について  
    - ガントチャート、スケジュール管理、タスク管理、設計、アーキテクチャ、要件定義、機能要件、非機能要件、ついでにフレームワークなど、実装以前の知識の部分で違いが分からない、どう進めればいいか分からない部分が多い。何を勉強し何を身に着ければいいのか、何から始めてどう実践するのがいいか分からない。コーディングスキル以外にも学習すること、足りない部分が多く見えいい経験になった。  
    - Obsidian、Ankiを上記の学習、取り組みにもっと活かしたい。  
...
[Pacman Game](https://pacman.live/play.html)
