...
gantt
    ...
    例外処理 (#47) :done, 2026-08-11, 2026-08-14
    README・PM文書 (#19) :done, 2026-08-13, 2026-08-15
```
...
|v1.0 仕上げ・提出|終盤|08-10~08-15 完了|計画通り|
...
|-|-|-|
|起用パッケージについて|`Pydantic`|ファイルの読み込みが必要だったのでまず最初に取り入れた。|
|グラフィックは**MLX相当**のみ|`pygame`を**画像貼付`blit`/ピクセル`set_at`/画像ロード/イベント**に限定。文字は`ImageFont`で画像連結|`pygame.draw.*`やAAフォントはMLX非対応で使用不可。|
||`Pygame`|ヴィジュアル化パッケージは当初モダン開発が可能なArcadeを使用予定だったが、PdfのアップデートによりMiniLibxと同等の機能のみ使用可能が要件に追加されたため、よりベーシックな描画ロジックを組むPygameが採用となった。|
...
**モデル設計**
- ほぼすべてのクラスが同一のメソッドと引数を持つことで共通のデータクラスを用意できた。あたらしいクラスを設計追加するとに１からデータを流し込んだり、整形したりする必要性を減り。分担する作業が明確になった。
...
|クラッシュ源(アセット欠落/生成失敗/空スコア)|提出不合格級|中|例外処理・コンテキストマネージャ。|一部残|
|デプロイ(Steam/Itch)未了|提出物欠落|中|PyInstaller spec・非公開ビルド(#20/#55で対応)|完了|
|2人チームのバス係数|属人化|中|PR相互レビューで知識共有|緩和|

>Windows（大文字・小文字を区別しない）とLinux/WSL（区別する）の違いによる、アセット読み込みエラー（Pacman と pacman の統合バグ）　-> 統一化し競合排除


## 5. チーム体制(担当・意思決定・問題対応)

- メンバーはイシューごとに作業ブランチを分ける -> メンバーとレビュー -> メインへとマージの流れで分担作業が行われた。  
- レビュー時に、次に行うべきイシューを二人で話し合った。お互いが最初に実装した機能から派生する近いイシューを選択した。  
- モデル設計の段階で見えてきたリリースまでのイシューを仮で設定した。イシュー単位で領域が離れているタスク（マージの際にコンフリクトしない浮いたタスク）が見つかりやすかったことで、片一方の作業がもう片方に依存して手が止まる瞬間がなく作業効率上昇につながった。
...
- メンバーレビュー前にAIレビューを通す習慣があった。当初から意図したものではなかったが、ブランチで作業した項目の洗い出しや影響範囲のリスク分析に役立った。  
...
|未処理例外でクラッシュしない|主要経路で確認|OK|#47|
|迷路生成(perfect=False)|1面固定シード、以降ランダム|OK|#6|
...
|内容|起票|修正|
|-|-|-|
|Level欠落でクラッシュ|#53|PR #51・#54系|
|ゴーストの当たり判定・速度|#42|PR #46|
|HUD実装でレベル無限アップ、時間が正確に表示されないバグを発見|#48|PR #50|
|チートモードでスピードアップ時にアイテムが取得できない||当たり判定をキャラクター外周のピクセル座標の一致からキャラクター中心部から外周間に存在するかの条件に変更|
...
- **マージ順ハザード**  
    #51 → #54 のマージ順で競合し、`fix: post merge main`で解消。

- 基本的に機能単位で分担していたため、コンフリクトはほぼ起きなかった。ミスでgit stash popを作業ブランチ外で行った際に、作業ブランチの変更が消えコンフリクトしたことがあったが、IDEの機能で対応した。  