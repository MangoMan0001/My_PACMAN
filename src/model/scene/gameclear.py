...
class GameClear(Scene):
    ...
    Attributes:
        ...
        game_clear_img (pygame.Surface): "GAME-CLEAR"のテキスト画像
    ...
    def __init__(self, config: ConfigModel, score: int, score_manager: ScoreManager):
        ...
        self.game_clear_img = title_font.render_text(game_clear_text)
    ...
    def draw(self, screen: pygame.Surface) -> None:
        ...
        _, ty = cx // 2, cy // 2
        _, zy = cx // 3, cy // 3
        title_x = cx - self.game_clear_img.get_width() // 2
        title_y = cy - self.game_clear_img.get_height() // 2
        ...
        screen.blit(self.game_clear_img, (title_x, title_y - ty))
        if not self.is_save:
